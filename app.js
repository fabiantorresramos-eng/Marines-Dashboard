/* Marine Proficiency Dashboard (Agony-theme)
   ...
   ✅ FIX INCLUDED:
   - Profile images are stored in the REPO ROOT (same folder as index.html)
   - All image URLs now use "./0631.png" etc (NOT ./assets/mos/...)
   - Robust fallback: if image fails (404), show initials instead of broken icon
*/
...
(function () {
  ...
  // Your PNG files live in the REPO ROOT:
  //   /0602.png /0621.png /0627.png /0631.png /0671.png /0699.png /1stsgt.png
  // so we load them as: "./0631.png"
  // ---------------------------------------------------------------------------
  const MOS_IMAGE_MAP = {
    "0602": "0602.png",
    "0621": "0621.png",
  ...
  function getProfileImageUrl(m) {
    ...
    const billetRaw = (m?.billet ?? m?.Billet ?? m?.title ?? m?.Title ?? "")
      .toString()
      .trim();
    const billet = billetRaw.toLowerCase();

    // Leadership artwork (also in root if you add them)
    if (billet.includes("company commander")) return "./company_commander.png";
    if (
      billet.includes("first sergeant") ||
      billet.includes("1stsgt") ||
      billet.includes("1st sgt")
    )
      return "./1stsgt.png";
  ...
  // Avatar helper: supports root-based images + safe fallback initials.
  function setAvatar(el, marine, fallbackText) {
    ...
    // If the image fails (404 / case mismatch), fallback to initials
    img.onerror = () => {
      el.classList.remove("has-img");
      el.textContent = fallbackText || "";
    };
  ...
  function groupRoster(roster) {
    ...
    for (const m of roster) {
      ...
      const team =
        m.team && String(m.team).trim()
          ? m.team
          : m.squad && String(m.squad).trim()
          ? "No Team"
          : "Unassigned";
    ...
    const sorted = [...root.entries()].sort(
      (a, b) => pltOrder(a[0]) - pltOrder(b[0]) || a[0].localeCompare(b[0])
    );
    ...
    for (const [, sMap] of sorted) {
      for (const [, tMap] of sMap) {
        for (const [, arr] of tMap) {
          arr.sort((x, y) =>
            `${x.lastName} ${x.firstName}`.localeCompare(
  ...
  function summarizeGroup(marines) {
    ...
    return { count, scored: count, overall };
  }
  ...
  function teamPanelData(ctx, members) {
    ...
    const sorted = members
      ...
      .sort(
        (a, b) =>
          overallForMarine(b) - overallForMarine(a) ||
  ...
  function radarSVG(rubric, ctx) {
    ...
    const rings = [0.25, 0.5, 0.75, 1].map((t) => {
      ...
      for (let i = 0; i < 5; i++) {
        const ang = -Math.PI / 2 + (i * (2 * Math.PI)) / 5;
        ringPts.push([cx + Math.cos(ang) * r * t, cy + Math.sin(ang) * r * t]);
      }
      ...
      <svg class="radar" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Radar chart">
        ...
        <g fill="none" stroke="rgba(246,243,243,.10)" stroke-width="1">
          ...
          ${spokes
            .map(
              (s) =>
                `<line x1="${cx}" y1="${cy}" x2="${s.x.toFixed(
                  1
                )}" y2="${s.y.toFixed(1)}"/>`
            )
            .join("")}
        ...
        ${pts
          .map((p) => {
            const attrs = `data-mid="${escapeXml(
              ctx.marineId
            )}" data-rid="${escapeXml(ctx.rubricId)}" data-ak="${escapeXml(
              p.key
            )}" data-score="${p.score}"`;
            return `
              <circle class="radar-hit radar-point" cx="${p.x.toFixed(
                1
              )}" cy="${p.y.toFixed(1)}" r="12" ${attrs} />
              <circle class="radar-dot radar-point" cx="${p.x.toFixed(
                1
              )}" cy="${p.y.toFixed(1)}" r="5" ${attrs} />
            `;
          })
          .join("")}
      </svg>
  ...
  function bigRadarSVG(rubric, ctx) {
    ...
    const labelEls = pts
      .map((p) => {
        ...
        return `<text class="radar-label" x="${lx.toFixed(
          1
        )}" y="${ly.toFixed(
          1
        )}" text-anchor="${anchor}" dominant-baseline="middle" font-size="12" fill="rgba(246,243,243,.88)">${escapeXml(
          lab
        )}</text>`;
      })
      .join("");
    ...
    const vbMargin = 84;
    return `
      <svg class="big-radar" viewBox="-${vbMargin} -${vbMargin} ${
      size + vbMargin * 2
    } ${size + vbMargin * 2}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Radar chart">
        <defs>
          <linearGradient id="${uid}" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0" stop-color="rgba(200,155,75,.95)"/>
        ...
        ${pts
          .map((p) => {
            ...
            return `
              <circle class="radar-hit radar-point" cx="${p.x.toFixed(
                1
              )}" cy="${p.y.toFixed(1)}" r="16" ${attrs} />
              <circle class="radar-dot radar-point" cx="${p.x.toFixed(
                1
              )}" cy="${p.y.toFixed(1)}" r="7" ${attrs} />
            `;
          })
          .join("")}
        ${labelEls}
  ...
  function statusClass(status) {
    ...
    if (s.includes("tad") || s.includes("leave") || s.includes("limdu") || s.includes("med"))
      return "s-warn";
    if (s.includes("eas") || s.includes("pcs") || s.includes("sep") || s.includes("terminal"))
      return "s-warn";
  ...
  function renderHeaderMeta({ status, operation }) {
    ...
    return `
      <span class="meta-badge ${statusClass(statusLabel)}"><span class="dot"></span> Status: ${escapeXml(
      statusLabel
    )}</span>
  ...
  function cardHTML(m) {
    ...
    const statusBadge = status
      ? `<span class="badge ${
          status.includes("TAD") || status.includes("LEAVE") || status.includes("MED")
            ? "warn"
            : "ok"
        }">${escapeXml(status)}</span>`
      : "";
    ...
    // ✅ avatar now uses ROOT image URLs + fallback-onerror
    ...
    const avatarInner = imgUrl
      ? `<img src="${escapeXml(imgUrl)}" alt="Profile artwork" loading="lazy" onerror="this.remove(); this.parentElement.classList.remove('has-img'); this.parentElement.textContent='${escapeXml(
          initials || "US"
        )}'">`
  ...
  function crumbsHTML(nav) {
    ...
    if (nav.plt)
      parts.push(
        `<span class="sep">›</span><button class="crumb" type="button" data-nav="plt">${escapeXml(
          nav.plt
        )}</button>`
      );
    if (nav.squad)
      parts.push(
        `<span class="sep">›</span><button class="crumb" type="button" data-nav="squad">Squad ${escapeXml(
          nav.squad
        )}</button>`
      );
    if (nav.team)
      parts.push(
        `<span class="sep">›</span><button class="crumb" type="button" data-nav="team">Team ${escapeXml(
          nav.team
        )}</button>`
      );
    return parts.join("");
  ...
  function renderPltOrgChart(container, marines, plt) {
    ...
    const staff = marines.filter(
      (m) => String(m.squad || "").trim().toLowerCase() === "platoon staff"
    );
      ...
      .map(([squad, tMap]) => {
        const teamCols = [...tMap.entries()]
          ...
          .map(([team, members]) => {
            ...
            const pd = teamPanelData({ plt: pltLabel, squad, team }, sorted);
            const pdJson = escapeXml(JSON.stringify(pd));
            return `
              <div class="org-team">
                <div class="org-team-title">${escapeXml(team)}
                  <button class="org-team-meta-btn team-panel-trigger" data-panel="${pdJson}">
                    ${sorted.length} · Avg ${pd.avg}%
                  </button>
                </div>
      ...
      <div class="org-wrap">
        ...
        ${
          ...
          <div class="org-staff">
            ...
            <div class="org-list">${staff
              .slice()
              .sort((a, b) => nameKey(a).localeCompare(nameKey(b)))
              .map((m) => nodeHTML(m, { compact: true }))
              .join("")}</div>
          </div>
        `
            : ""
  ...
  function renderHierarchy(container, roster, nav, crumbsEl) {
    ...
    const pltScoped = roster.filter(
      (m) => String(m.plt || "HQ").trim() === String(nav.plt).trim()
    );
  ...
  function buildProfileModal() {
    ...
    return {
      ...
      open: (marine, openRubricId) => {
        const name =
          `${marine.rank || ""} ${marine.firstName || ""} ${marine.lastName || ""}`.trim() ||
          "Marine";

        $("#mdName", el).textContent = name;
        $("#mdSub", el).textContent = `${marine.plt || "HQ"} · Squad ${
          marine.squad || "Unassigned"
        } · Team ${marine.team || "Unassigned"} · MOS ${marine.mos || "—"}`;
        ...
        const kv = (k, v) =>
          `<div class="info-row"><span class="k">${escapeXml(
            k
          )}</span><span class="v">${escapeXml((v ?? "").toString().trim() || "—")}</span></div>`;
        ...
        const opsHTML = () => {
          const rows = [
            ...
            kv(
              "Current Op Start",
              marine.currentOpStart ||
                rf(["Current Operation : Start Time", "Current Operation  : Start Time"])
            ),
            kv(
              "Current Op End",
              marine.currentOpEnd ||
                rf(["Current Operation : End Time", "Current Operation  : End Time"])
            ),
            kv("Future Operation", marine.futureOperation || rf(["Future Operation"])),
            kv(
              "Future Op Start",
              marine.futureOpStart ||
                rf(["Future Operation: Start Time", "Future Operation : Start Time"])
            ),
            kv(
              "Future Op End",
              marine.futureOpEnd ||
                rf(["Future Operation: End Time", "Future Operation : End Time"])
            ),
          ].join("");
          return `<div class="info-grid">${rows}</div>`;
        };
        ...
        if (pTabsEl && pContentEl) {
          pTabsEl.innerHTML = profileTabs
            .map((t, i) => `<button class="ptab hud-chip ${i === 0 ? "active" : ""}" data-pt="${t.id}">${escapeXml(t.label)}</button>`)
            .join("");
        ...
        // ✅ Avatar image load uses fixed root path + fallback
        const initials = `${(marine.firstName || "")[0] || ""}${(marine.lastName || "")[0] || ""}`.toUpperCase();
        setAvatarEl($("#mdAvatar", el), { mos: marine.mos, billet: marine.billet }, initials || "US");
        ...
        tabs.innerHTML = RUBRIC_KEYS.map((rid) => {
          ...
          return `<button class="tab hud-chip ${active ? "active" : ""}" data-r="${escapeXml(
            rid
          )}">${escapeXml(RUBRIC_LABELS[rid] || rid)}</button>`;
        }).join("");
  ...
  function buildAreaModal() {
    ...
    return {
      ...
      open: ({ marine, rubricId, areaKey }, onOpenProfile) => {
        const initials = `${(marine.firstName || "")[0] || ""}${(marine.lastName || "")[0] || ""}`.toUpperCase();
        setAvatarEl($("#adAvatar", el), { mos: marine.mos, billet: marine.billet }, initials || "US");
        ...
        $("#adSub", el).textContent = `${marine.rank || ""} ${marine.firstName || ""} ${marine.lastName || ""} · ${
          RUBRIC_LABELS[rubricId] || rubricId
        }`.trim();
        ...
        $("#adMeta", el).textContent =
          [last, who].filter(Boolean).join(" · ") ||
          "Tip: add *_notes, *_lastEval, *_evaluator columns to your CSV to enrich this view.";

        const btn = $("#adOpenProfile", el);
        btn.onclick = () => {
          close();
          onOpenProfile?.(marine, rubricId);
        };
  ...
  function buildTeamPanel() {
    ...
    return {
      ...
      open: (data) => {
        ...
        $("#tpBottom3", el).innerHTML =
          (data.bottom3 || []).map(rowHTML).join("") || `<div class="small">—</div>`;
        el.classList.add("show");
      },
      close,
    };
  ...
  function getField(row, names) {
    for (const n of names) {
      ...
      const found = Object.keys(row).find(
        (k) => k.trim().toLowerCase() === String(n).trim().toLowerCase()
      );
  ...
  function getAreaValueFromRow(row, rubricId, areaKey) {
    ...
    const mosSet = (String(rubricId || "").match(/\d{3,4}/g) || []).map((m) =>
      String(m).padStart(4, "0")
    );
    const areaTitles = {
  ...
  function rowToMarine(row, idx = 0) {
    ...
    const currentOpStart = getField(row, [
      "Current Operation : Start Time",
      "Current Operation  : Start Time",
      "Current Operation Start Time",
      "Current Op Start",
    ]);
    const currentOpEnd = getField(row, [
      "Current Operation : End Time",
      "Current Operation  : End Time",
      "Current Operation End Time",
      "Current Op End",
    ]);
  ...
  async function mount(rootSel, opts = {}) {
    ...
    function wireInteractions() {
      ...
      // Radar point click opens area modal (delegated)
      if (!document.__md_radar_delegate) {
        document.__md_radar_delegate = true;
        document.addEventListener(
          "click",
          (e) => {
            const target = e.target?.closest ? e.target.closest(".radar-point") : e.target;
    ...
    $("#mdSearch", root).addEventListener("input", (e) => {
